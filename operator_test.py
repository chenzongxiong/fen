...
def generate_play_operator(weight=1.0, width=1.0, method="integer"):
    ...
    if method == "integer":
        ...
        _inputs1 = np.sin(np.linspace(-np.pi, np.pi, points))
        _inputs2 = 3 * np.sin(np.linspace(-np.pi, np.pi, points))
        _inputs3 = 1.2 * np.sin(np.linspace(-np.pi, np.pi, points))
        _inputs = np.hstack([_inputs1, _inputs2, _inputs3])
...
def fit(inputs, outputs, width, method, true_weight):
    ...
    with open("./training-data/operators/pavel-trained-{}-{}-{}.txt".format(method, true_weight, width), "w") as f:
    ...
    fname = "./training-data/operators/pavel-pred-{}-{}-{}.csv".format(method, true_weight, width)
...
if __name__ == '__main__':
    ...
    #             fname = "./training-data/operators/pavel-{}-{}-{}.csv".format(method, weight, width)
    #             save(inputs, outputs, fname)
    #             if save_anim is True:
    #                 fname = "./pics/operators/pavel-{}-{}-{}.gif".format(method, weight, width)
